...
> 本构建新增了成本优化的子 Agent 模型路由（modelAlias）和按配置文件的思考深度（thinkingEffort）配置。自动更新和上游反馈功能已禁用。详情见下方 [自定义特性](#自定义特性) 章节。
...
- **按配置文件的思考深度（thinkingEffort）配置。** 每个子 Agent 配置可以声明独立的 `thinkingEffort`（如 `off` / `low` / `medium` / `high` / `max`），与主 Agent 的设置无关。便于按任务类型精细调整推理深度。
...
也可以通过创建自定义 Agent 配置文件（YAML）并声明 `modelAlias` 和 `thinkingEffort` 字段来覆盖。配置加载器支持 `extends` 继承和按子 Agent 条目覆盖。
